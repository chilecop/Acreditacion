...
<html lang="en">
  <head>
    ...
    if(isset($_GET['id'])){
      $id = $_GET['id'];
      $datos = getDatosVehiculo($id);
      list($patente,$conductor,$marca,$modelo,$color,$anio,$tipo) = explode("%$",$datos);
    }
    ?>
        ...
        $('#formulario').submit(function() {
          $.ajax({
            ...
            success: function(data) {
              if(data=='ok'){
                $('#respuesta').html("Vehiculo modificado correctamente.");
              }else{
                $('#respuesta').html("");
                alert(data);
              }
    ...
    <div class="container-fluid display-table">
      <div class="row display-table-row">
        ...
        <div class="col-md-10 col-sm-11 display-table-cell valign-top">
          ...
          <div id="content">
            <header>
              <h2 class="page_title">Editar Vehiculo #<?php echo $id;?></h2>
            </header>
            <div class="content-inner">
              <div class="form-wrapper">
              <div id="respuesta"></div>
                <form id="formulario" action="php/editarVehiculo.php" method="post">
                <input type="hidden" name="id" value="<?php echo $id;?>">
                  <div class="row">
                    <div class="col-md-6 col-lg-6">
                      <div class="form-group">
                        <label class="sr-only">Patente</label>
                        <input type="text" class="form-control" id="title" placeholder="Patente" name="patente" value="<?php echo $patente; ?>" required>
                      </div>
                      <div class="form-group">
                        <label class="sr-only">Rut Conductor</label>
                        <input type="text" class="form-control" id="title" placeholder="Rut Conductor" name="rut" value="<?php echo $conductor; ?>" required>
                      </div>
                      <div class="form-group">
                        <label class="sr-only">Marca</label>
                        <input type="text" class="form-control" id="title" placeholder="Marca" name="marca" value="<?php echo $marca; ?>" required>
                      </div>
                      <div class="form-group">
                        <label class="sr-only">Modelo</label>
                        <input type="text" class="form-control" id="title" placeholder="Modelo" name="modelo" value="<?php echo $modelo; ?>" required>
                      </div>
                    </div>
                    <div class="col-md-5 col-lg-5">
                      <div class="form-group">
                        <label class="sr-only">Tipo</label>
                        <?php getTipoVehiculoSelect($tipo);?>
                      </div>
                      <div class="form-group">
                        <label class="sr-only">Color</label>
                        <input type="text" class="form-control" id="title" placeholder="Color" name="color" value="<?php echo $color; ?>">
                      </div>
                      <div class="form-group">
                        <label class="sr-only">Año</label>
                        <input type="text" class="form-control" id="title" placeholder="Año" name="anio" value="<?php echo $anio; ?>">
                      </div>
                      
                      <div class="clearfix">
                        <button type="submit" class="btn btn-warning pull-right"> Editar Vehiculo</button>
                      </div>
                    </div>
                  </div>
                </form>
